...
**Answer**
amount is <dbl> (double floating point)
name is <chr> (character string)
id is <dbl> (double floating point)
```{r}
...
**Answer**:
275 records with franchise name Subway
```{r}
...
**Answer**:
All the records with no forgiveness date also have an NA forgiveness amount.
```{r}
ppp_maryland_loans %>%
...
**Answer**
THe largest is not suspicious because that is the maximum PPP loan amount.
The minimum looks questionable to me. Why would someone apply for a $6 loan? Unless they want to go to Starbucks.
```{r}
...
**Answer**
Conflicting unique data (duplicated record with id 93761495)
Consistency with strett address (case differences)
unit/format inconsistemcy (zip)
wrong type (amount is <chr>, can't be aggregated)
```{r}
arnold_md_loans
```
...
changed the name of the 1_id column to x1_id
```{r}
# cleaning function
...
**Answer**
amount is not summable because it is not a numeric field
```{r}
# cleaning function
