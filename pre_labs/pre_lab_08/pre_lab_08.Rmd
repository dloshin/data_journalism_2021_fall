...
```{r echo=FALSE}
census_api_key("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", install=TRUE)
```
...
```{r}
p00 <- get_decennial(geography = "state", variables = "P001001", year = 2000, key = "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
p10 <- get_decennial(geography = "state", variables = "P001001", year = 2010, key = "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
```
...
**Answer**
Well, the bars overlap and you said that if they overlap the differences aren't statistically significant so I say no.
Is the difference between Howard and everyone else significant?
**Answer**
It doesn't overlpa so I would say yes.
...
**Answer**
Somerset and Caroline counties.
SOmerset no, Caroline, yes. (I think. or maybe the other way around?()
```{r}
md10 %>%
