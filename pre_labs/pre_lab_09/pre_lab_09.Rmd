...
**Answer**
Grouing by county names then counting the number of loans in each county and sorting in descending order and taking the top 10.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
Bar chart, unordered columns, names smooshed together.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Answer**
same data chart with bars ordered in ascending order to the right.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Answer**
Different font and blue background and the chart grid is different.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
Chart with the labels we added.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Dataframe with dates and the number of loans approved on that date.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Line chart, big spike early (probably the first setof loans), then others, then a lull after the intial money ran out, then more activity for second draw loans.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Same chart but with a tick mark for the beginning of each month.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
Ooh much nicer, I can actually read that.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
