...
```{r, echo=FALSE}
knitr::include_graphics(rep("images/iOLAz-2021-ppp-loan-volume-by-lender-type.png"))
```
...
aggregating counts of loans by county
```{r}
...
**Answer** Describe what the code below is doing and the output.
Getting rid of those couties with fewer than 7 loans
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
The combined state and county codes into a single 5-digit code
```{r}
...
JOINed table with loan counts by county/FIPS
```{r}
...
limits it to just Maryland but it still looks a little weird.
...
**Answer** Describe the changes from the previous map
Now says the county name and a comma'd count.
...
```{r, echo=FALSE}
knitr::include_graphics(rep("images/NFYcd-businesses-in-montgomery-and-prince-georges-counties-take-bulk-of-ppp-loans.png"))
```
