...
**Answer**
OBJECTID, ID, NAME, ADDRESS, CITY, STATE, ZIP, ZIP4, TELEPHONE, TYPE, STATUS, POPULATION, COUNTY, COUNTYFIPS, COUNTRY, LATITUDE, LONGITUDE, NAICS, NAICS, SOURCE, SOURCEDATE, VAL, VAL, WEBSITE, STATE, ALT, ST, OWNER, TTL, BEDS, TRAUMA, HELIPAD, geometry
```{r}
...
**Answer**
It automatically formats the shape data adn presents a text version of it.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A list of 7 types of hospitals, their count, and a reference to their grometry.
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A representation of the first 10 rows of the dataframe with maryland acute care hospitals.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Mapped locations of hospitals but no underlying map!
```{r}
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
It shows a dataframe, which I presume has information about the counies in the US? With ploygons instead of points?
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The points of the hospitals layered on top of the Maryland county map!
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Similar to priot map but now magnitude of beds is displayed by color spectrum
```{r}
...
**Answer**
It says that fips_codes is masked from tigris. Is that a problem?
```{r}
library(tidyverse)
library(sf)
library(janitor)
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
It loaded the maryland hospital data into a dataframe.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Dataframe with tally of total hospital beds by jurisdiction.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Populations by county and FIPS code.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
dataframe with population *and* shape data.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
this code block just looks at the data and validates that the values are not JOINable
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Reduced dataframe, cleaned up so the values can be JOINed, and limited the columns to the ones we will want.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The total number of beds per county is added in as a new column.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Added a new column with beds per 100K population.
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Noice, map with counties shaded according to beds per 100K!
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Prettier. I bet there might be better shading to be done since most of the map seems to be the same color.
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
...
**Answer**
Yes, that is a lot better differentiation between the beds/100K
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
