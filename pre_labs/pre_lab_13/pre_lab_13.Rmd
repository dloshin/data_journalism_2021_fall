...
**Answer**
A dataframe with states ordered by the percentage of businesses receiving PPE (Is it PPE - protective equipment - or PPP???)
```{r}
...
**Answer**
The trend seems to indicate a positive correlation between trumpiness and percentage of businesses getting loans.
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
Seems that there is a (less) positive correlation between trumpiness and average amount provided per employee. However that does not necessarily indicate causation but rather attributes of the types of businesses and the kinds of salaries their employees get, and location may have a causal effect on that.
```{r}
...
**Answer**
Pretty strong correlation between trumpiness and percentage getting loans, less so for amount per employee.
```{r}
reveal_data %>%
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**
Higher p value, above the standard .05, 95% confidence interval is pretty broad.
```{r}
...
**Answer**
2 sample t-test results for amount per employee. P value is pretty high.
```{r}
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
Two sample t test results for percentage of businesses receiving ppp. Much smaller p value.
```{r}
